...
void nexorEncrypt(char *keys, unsigned int keyNum, FILE *infile, FILE *outfile) {
	...
	long long unsigned int infileSize = ftello(infile);
	#ifdef DEBUG
	...
	while (1) {
		...
		if (ftello(infile) == infileSize) {
			break;
		}

		block[b] = fgetc(infile);
		if (((r+1)%16 == 0 && r>0) || r+1 == infileSize) {
...
void nexorDecrypt(char *keys, unsigned int keyNum, FILE *infile, FILE *outfile) {
...
int main(int argc, char *argv[]) {
	...
	// get the keyfile size
	fseeko(keyfp, 0, SEEK_END);
	unsigned long long keyfileSize = ftello(keyfp);
	fseeko(keyfp, 0, SEEK_SET);
	...
	unsigned int keyNum = nexorOpenKeyfile(keyfp, keys);
	...
	if (strncmp(argv[1], "enc", 3) == 0) {
		// this is encrypt
		nexorEncrypt(keys, keyNum, infile, outfile);
	} else {
		// this is decrypt
		nexorDecrypt(keys, keyNum, infile, outfile);
	}
